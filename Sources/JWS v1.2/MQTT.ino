...
void connexion_MQTT() {
  ...
  if (!client.connected()) {           // Reconnexion MQTT si nécessaire
      compteur_echecMQTT++;
      if (compteur_echecMQTT == 1) {
        ledcFade(LED_CNXMQTT, LED_OFF, LED_lummax, 500); // Signale l'erreur de connexion MQTT avec la LED dédiée
        delay(500);
      }
      ...
      if (now - lastReconnectAttempt > 5000) {
       ...
       // Tentative de reconnexion
        if (reconnect()) {             // Connexion MQTT OK
          ...
          ledcWrite(LED_CNXMQTT, LED_OFF);    // Eteint la LED d'erreur de connexion MQTT 
          }
      }
    } else {
      compteur_echecMQTT = 0;
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[1] == 'o') {
    ...
    ledcWrite(LED_MSGMQTT, LED_OFF);    // Eteint la LED d'erreur de message MQTT     
    ...
    { compteur_echecExRx++;
      if (compteur_echecExRx == 1) {
        ledcFade(LED_MSGMQTT, LED_OFF, LED_lummax, 500); // Signale l'erreur de message MQTT avec la LED dédiée
        delay(500);
      }
