...
*                                                                                                                         *
*      Auteur : Daniel Jung                                                                                               *
*      Développement v1.31  : 15 mars 2025                                                                                *
...
* v1.11 - 02 Nov 2024  - Alongement du cycle à une minute pour permettre l'accomplissement des mises à jour sur Jeedom    *
* v1.2  - 09 Nov 2024  - Ajout d'un test de la connectivité Wifi du JWS au réseau local, avec tentative de reconnexion et *
*                        reboot le cas échéant,                                                                           * 
*                      - Définition du nom d'hôte 'JWS_ESP32' pour l'identification sur le réseau Wifi paramétré.         * 
* v1.3  - 20 Jan 2025  - Possibilité de suspendre la surveillance de la box Jeedom (passage en mode maintenance).         *
*                      - Amélioration des interfaces web                                                                  *
* v1.31 - 15 Mar 2025  - Augmentation du time-out pour établir une connexion Wifi si les identifiants sont renseignés.    *
*                                                                                                                         *
***************************************************************************************************************************
...
La récurrence des tests effectués est de 60 secondes. Si l'un des résultats remonte une défaillance, le JWS va déclarer une 
anomalie (affichage de KO + LEDs) et réitérer les tests jusqu'à cinq fois, soit un délai de 5 minutes accordé à la box Jeedom  
pour corriger un éventuel problème ou faire ses mises à jour, avant une activation du relais pendant 0,5 secondes provoquant 
un arrêt/marche.
Après deux activations consécutives, le relais est désactivé tant que le(s) problème(s) ne sera(ont) pas résolu(s) par
ailleurs.
Dans la page html des paramètres, deux boutons 'Opérationnel' et 'Maintenance' permettent de basculer entre le mode 
opérationnel (le mode normal de fonctionnement) qui assure la surveillance active de la box Jeedom, et le mode maintenance
où la surveillance de la box Jeedom est suspendue sans aucune action correctrice (pour les mises à jour longues, etc...). 
...
En fonctionnement : erreur de messages MQTT                  | 1x toutes les 30" = JWS en service
                                                             | En permanence: la box Jeedom n'est plus surveillée
                                                             | (mode maintenance)
----------------------------------------------------------------------------------------------------------------
...
6.- Connexion au réseau WiFi et auto-surveillance

A l'initialisation, le JWS va se placer en mode point d'accès (PA) si aucun identifiant (SSID et mot de passe) n'a été 
enregistré au préalable (ce qui est le cas au premier démarrage).
Si un identifiant a déjà été renseigné, une fois la connexion au réseau Wifi réalisée, le JWS va vérifier en permanence la 
connectivité sur ce réseau Wifi. 
En cas de perte de connexion du réseau, le JWS va rebooter et va tenter de se reconnecter toute les secondes sur le réseau
enregistré pendant une période de 3'20" maximum. S'il y a toujours un échec de connexion à l'issue, le JWS va se placer en
mode point d'accès. 
*/

#include <ESPping.h>                   // Gestion des ping IP
#include <PubSubClient.h>              // Gestion réseau MQTT
#include <WebServer.h>                 // Création d'un portail web pour la gestion du mode Wifi AP (Access Point), du paramétrage et des mises à jour
...
#include "html_para.h"                 // Gestion page html des paramètres
#include "html_AP.h"                   // Gestion page html de la définition des paramètres en mode AP
...
#define delai_boucle   60000           // Délai entre chaque boucle de tests (60000 = 60 secondes)
#define Version        "1.31"          // Version logicielle actuelle
...
bool checkboxState = false;            // Variable pour stocker l'état de la case à cocher du mode maintenance de la box jeedom 
...
//   2.- Pour le mode STA
const char* hostname_st = "JWS_ESP32"; // Nom d'hôte affiché sur le réseau Wifi défini (mode ST)
...
bool mode=0;                           // Mode dans lequel est déclaré la box Jeedom : 0 = opérationnel, 1 = maintenance
String buttonState = "Opérationnel";   // Libéllé du mode dans lequel est déclaré la box Jeedom : Opérationnel, Maintenance
...
const int resolution = 8;              // Résolution à 8 bits (plage de 0 à 255)
...
#if Type_LED_CC                        // LED RGB installée de type Cathode Commune
  const int LED_OFF = 0;               // Codage de la valeur pour éteindre les LEDs de type Cathode Commune (CC)
  const int LED_ON = 255;              // Codage de la valeur pour allumer à 100% les LEDs de type Cathode Commune (CC)
  int LED_lummax = 200;                // Luminosité maximale des LED (0 = éteinte, 255 = maximale)
  bool ledState = 0;                   // Etat initial des LED (éteintes)
#endif

#if Type_LED_AC                        // LED RGB installée de type Anode Commune
  ...
  int LED_lummax = 50;                 // Luminosité maximale des LED (255 = éteinte, 0 = maximale)
...
void setup() {
  ...
  esp_task_wdt_config_t wdt_config = {
        .timeout_ms = 120000,          // Timeout pour les tâches fixé à 120 secondes max
...
void loop() {
  ...
  server.handleClient();
  if (WiFi.getMode() == WIFI_STA) {    // Les tests ne sont effectués qu'en mode STA (connecté à un PA)
    
    WiFiConnect();                     // Vérification de la connexion WiFi, et reconnexion si besoin
    ArduinoOTA.handle();               // Permet de gérer les mises à jour OTA par l'IDE Arduino en arrière-plan
    if (!mode) {                       // Surveillance active de la box Jeedom (mode opérationnel)
    ...
    } else                             // Surveillance suspendue de la box Jeedom (mode maintenance) 
      {
        Affichage_MTN();               // Affichage de l'écran mode Maintenance
        ledcFade(LED_MSGMQTT, LED_OFF, LED_lummax, 500); // Pulsations de la LED MSGMQTT
        delay(500);
        ledcFade(LED_MSGMQTT, LED_lummax, LED_OFF, 500);  
        delay(500);
      }
