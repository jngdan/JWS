...
#define SCREEN_ADDRESS   0x3c          // Voir le datasheet pour la définition de l'adresse : 0x3D pour 128x32, 0x3C pour
                                       // 128x64
#define TIMER_PAGE      20000          // Défini le nombre de secondes (en ms) pendant lequel l'écran restera affiché 
...
void InitOLED(){
  ...
  if(OLED_RESET != -1) {
    ...
    delay(250);                        // Attente du réveil de l'écran
...
// ----------------------------------------------------------------------
// Affichage du logo à l'initialisation de l'écran OLED
// ----------------------------------------------------------------------
 void OLED_Logo() {

  display.clearDisplay();
  display.setCursor(0,0);
  display.drawBitmap(0, 0, Logo_JWS, 128, 68, OLED_WHITE);
  display.display();
 }
